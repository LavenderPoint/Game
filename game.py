...
IS_MOBILE = "ANDROID_ARGUMENT" in os.environ or "iOS" in os.environ

# Константы для мобильных устройств
SCREEN_WIDTH = 360 if IS_MOBILE else 1200
SCREEN_HEIGHT = 640 if IS_MOBILE else 800
...
TITLE_FONT_SIZE = 36 if IS_MOBILE else 48
HEADER_FONT_SIZE = 28 if IS_MOBILE else 32
NORMAL_FONT_SIZE = 20 if IS_MOBILE else 24
SMALL_FONT_SIZE = 16 if IS_MOBILE else 20
TASK_SIZE = 25 if IS_MOBILE else 30
SCROLL_SPEED = 20
...
normal_font = pygame.font.SysFont("Arial", NORMAL_FONT_SIZE)
...
class Player:
    ...
    def generate_avatar(self):
        ...
        font_size = HEADER_FONT_SIZE - 4 if IS_MOBILE else HEADER_FONT_SIZE
...
class Game:
    def __init__(self):
        ...
        self.category_scroll = [0, 0, 0, 0]  # Прокрутка для каждой категории
        self.active_category = None  # Активная категория для прокрутки
        self.scroll_start_x = 0  # Начальная позиция прокрутки
    ...
    def setup_tasks(self):
        ...
        self.tasks_remaining = self.total_tasks  # Изначально все задания доступны
...
def draw_setup_screen(game):
    ...
    # Панель настройки
    panel_height = 400 if IS_MOBILE else 500
    pygame.draw.rect(screen, PANEL_COLOR, (20, 80, SCREEN_WIDTH - 40, panel_height), border_radius=20)
    pygame.draw.rect(screen, TEXT_COLOR, (20, 80, SCREEN_WIDTH - 40, panel_height), 2, border_radius=20)
    
    # Заголовок панели
    setup_title = header_font.render("Настройка игры", True, TEXT_COLOR)
    screen.blit(setup_title, (SCREEN_WIDTH // 2 - setup_title.get_width() // 2, 100))
    
    # Ввод количества заданий
    tasks_label = normal_font.render("Заданий в категории (5-50):", True, TEXT_COLOR)
    screen.blit(tasks_label, (40, 150))
    
    # Ввод имени игрока
    player_label = normal_font.render("Имя игрока:", True, TEXT_COLOR)
    screen.blit(player_label, (40, 220))
    
    # Информация об игроках
    players_label = normal_font.render("Добавленные игроки:", True, TEXT_COLOR)
    screen.blit(players_label, (40, 290))
    
    # Список игроков
    for i, player in enumerate(game.players):
        player_text = small_font.render(f"{i+1}. {player.name}", True, TEXT_COLOR)
        screen.blit(player_text, (60, 330 + i * 30))
...
def draw_game_screen(game):
    ...
    pygame.draw.rect(screen, (200, 200, 200), (10, 60, SCREEN_WIDTH - 20, 15))
    pygame.draw.rect(screen, (0, 150, 0), (10, 60, (SCREEN_WIDTH - 20) * progress, 15))
    progress_text = small_font.render(f"{game.completed_tasks}/{game.total_tasks} ({int(progress*100)}%)", True, TEXT_COLOR)
    screen.blit(progress_text, (SCREEN_WIDTH // 2 - progress_text.get_width() // 2, 62))
    
    # Информация о текущем игроке
    if game.players:
        current_player = game.players[game.current_player_idx]
        player_text = header_font.render(f"Игрок: {current_player.name}", True, current_player.color)
        screen.blit(player_text, (10, 85))
    
    # Панель категорий
    category_height = (SCREEN_HEIGHT - 120) // 4 - 10
    for i, category in enumerate(game.categories):
        y_pos = 120 + i * (category_height + 10)
        pygame.draw.rect(screen, CATEGORY_COLORS[i], (10, y_pos, SCREEN_WIDTH - 20, category_height), border_radius=10)
        pygame.draw.rect(screen, TEXT_COLOR, (10, y_pos, SCREEN_WIDTH - 20, category_height), 2, border_radius=10)
        
        # Название категории
        cat_text = header_font.render(category, True, TEXT_COLOR)
        screen.blit(cat_text, (20, y_pos + 10))
        
        # Задания (горизонтальная прокрутка)
        tasks_start_x = 20 - game.category_scroll[i]
        for j in range(game.tasks_per_category):
            task_x = tasks_start_x + j * (TASK_SIZE + 10)
            task_y = y_pos + category_height // 2
            
            # Пропускаем задания за пределами экрана
            if task_x + TASK_SIZE < 20 or task_x > SCREEN_WIDTH - 20:
                continue
                
            task = game.tasks[i][j]
            color = (100, 200, 100) if task["completed"] else CATEGORY_COLORS[i]
            pygame.draw.circle(screen, color, (task_x, task_y), TASK_SIZE // 2)
            pygame.draw.circle(screen, TEXT_COLOR, (task_x, task_y), TASK_SIZE // 2, 1)
...
def draw_task_window(game):
    ...
    title = header_font.render(f"Уровень {task['difficulty']}", True, TEXT_COLOR)
    screen.blit(title, (window_x + window_width // 2 - title.get_width() // 2, window_y + 20))
    
    # Категория
    cat_text = normal_font.render(f"Категория: {game.categories[cat_idx]}", True, CATEGORY_COLORS[cat_idx])
    screen.blit(cat_text, (window_x + 20, window_y + 60))
    
    # Описание задания
    desc_text = normal_font.render("Задание:", True, TEXT_COLOR)
    screen.blit(desc_text, (window_x + 20, window_y + 100))
    ...
    for word in words:
        ...
        if normal_font.size(test_line)[0] < window_width - 60:
    ...
    for i, line in enumerate(lines):
        ...
        screen.blit(line_surf, (window_x + 40, window_y + 140 + i * 30))
    
    # Показываем ответ только если нажали кнопку "Показать ответ"
    y_pos = window_y + 140 + len(lines) * 30 + 20
    button_width = 160
    button_height = 50
    button_spacing = 10
    ...
    if cat_idx == 0:  # Загадки
        if game.show_answer:
            # Показываем ответ
            ans_text = normal_font.render(f"Ответ: {task['answer']}", True, (0, 100, 0))
            screen.blit(ans_text, (window_x + 30, y_pos))
            y_pos += 40
...
def draw_intermediate_results(game):
    ...
    # Заголовок
    title = title_font.render(f"Результаты: Раунд {game.round_counter}", True, (220, 20, 60))
    screen.blit(title, (SCREEN_WIDTH // 2 - title.get_width() // 2, 20))
    
    # Информация о прогрессе
    progress = game.completed_tasks / game.total_tasks
    progress_text = header_font.render(f"Выполнено: {int(progress*100)}%", True, TEXT_COLOR)
    screen.blit(progress_text, (SCREEN_WIDTH // 2 - progress_text.get_width() // 2, 80))
    
    # Таблица результатов
    pygame.draw.rect(screen, PANEL_COLOR, (20, 120, SCREEN_WIDTH - 40, SCREEN_HEIGHT - 200), border_radius=20)
    pygame.draw.rect(screen, TEXT_COLOR, (20, 120, SCREEN_WIDTH - 40, SCREEN_HEIGHT - 200), 2, border_radius=20)
    
    # Список всех игроков
    sorted_players = sorted(game.players, key=lambda p: p.score, reverse=True)
    for i, player in enumerate(sorted_players):
        # Аватар
        screen.blit(player.avatar, (40, 150 + i * 70))
        
        # Имя и очки
        name_text = normal_font.render(f"{i+1}. {player.name}", True, player.color)
        screen.blit(name_text, (120, 160 + i * 70))
        
        score_text = header_font.render(f"{player.score}", True, TEXT_COLOR)
        screen.blit(score_text, (SCREEN_WIDTH - 80 - score_text.get_width(), 160 + i * 70))
...
def draw_game_over_screen(game):
    ...
    # Заголовок
    title = title_font.render("Игра завершена!", True, (220, 20, 60))
    screen.blit(title, (SCREEN_WIDTH // 2 - title.get_width() // 2, 20))
    
    # Пьедестал для топ-3
    podium_height = 200
    podium_width = SCREEN_WIDTH - 40
    podium_x = 20
    podium_y = 80
    ...
    for i, player in enumerate(sorted_players[:3]):
        ...
        avatar = pygame.transform.scale(player.avatar, (80, 80))
        avatar_rect = avatar.get_rect(center=(x_pos, y_pos - 40))
        screen.blit(avatar, avatar_rect)
        
        # Имя
        name_text = normal_font.render(player.name, True, player.color)
        screen.blit(name_text, (x_pos - name_text.get_width() // 2, y_pos + 10))
        
        # Очки
        score_text = normal_font.render(f"{player.score} очков", True, TEXT_COLOR)
        screen.blit(score_text, (x_pos - score_text.get_width() // 2, y_pos + 40))
    
    # Таблица для остальных игроков
    if len(sorted_players) > 3:
        other_y = podium_y + podium_height + 20
        other_height = 120 if len(sorted_players) <= 6 else 180
        pygame.draw.rect(screen, PANEL_COLOR, (20, other_y, SCREEN_WIDTH - 40, other_height), border_radius=20)
        pygame.draw.rect(screen, TEXT_COLOR, (20, other_y, SCREEN_WIDTH - 40, other_height), 2, border_radius=20)
        
        other_title = header_font.render("Остальные участники", True, TEXT_COLOR)
        screen.blit(other_title, (SCREEN_WIDTH // 2 - other_title.get_width() // 2, other_y + 10))
        
        # Отображение остальных игроков
        for i, player in enumerate(sorted_players[3:]):
            idx = i + 4
            y_pos = other_y + 50 + i * 30
            # Аватар
            screen.blit(player.avatar, (40, y_pos - 10))
            
            # Имя и очки
            player_text = normal_font.render(f"{idx}. {player.name}: {player.score}", True, player.color)
            screen.blit(player_text, (100, y_pos))
...
def draw_virtual_buttons():
    # Кнопка "Назад"
    pygame.draw.rect(screen, (100, 200, 100), (10, 10, 80, 40), border_radius=5)
    back_text = small_font.render("Назад", True, (255, 255, 255))
    screen.blit(back_text, (50 - back_text.get_width()//2, 30 - back_text.get_height()//2))
    
    # Кнопка "Меню"
    pygame.draw.rect(screen, (200, 100, 100), (SCREEN_WIDTH - 90, 10, 80, 40), border_radius=5)
    menu_text = small_font.render("Меню", True, (255, 255, 255))
    screen.blit(menu_text, (SCREEN_WIDTH - 50 - menu_text.get_width()//2, 30 - menu_text.get_height()//2))

# Создание объектов игры и UI
game = Game()

# Кнопки для экрана настройки
start_btn = Button(SCREEN_WIDTH // 2 - 100, 450 if IS_MOBILE else 600, 200, 50, "Начать игру")
add_player_btn = Button(SCREEN_WIDTH // 2 - 100, 250 if IS_MOBILE else 380, 200, 50, "Добавить игрока")

# Поля ввода
tasks_input = InputBox(SCREEN_WIDTH // 2 - 100, 180, 200, 40)
player_input = InputBox(SCREEN_WIDTH // 2 - 100, 250, 200, 40)

# Кнопки для окна задания
show_answer_btn = Button(0, 0, 160, 50, "Показать ответ")
accept_btn = Button(0, 0, 160, 50, "Принять", (144, 238, 144))
reject_btn = Button(0, 0, 160, 50, "Отклонить", (255, 99, 71))

# Кнопки для других экранов
continue_btn = Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT - 80, 200, 50, "Продолжить")
new_game_btn = Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT - 80, 200, 50, "Новая игра")
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if IS_MOBILE and event.type == MOUSEBUTTONDOWN:
            # Кнопка "Назад"
            ...
            # Кнопка "Меню"
            elif SCREEN_WIDTH - 90 <= mouse_pos[0] <= SCREEN_WIDTH - 10 and 10 <= mouse_pos[1] <= 50:
        ...
        if game.state == "SETUP":
            ...
            # Обработка прокрутки
            if event.type == MOUSEBUTTONDOWN:
                # Проверяем нажатие на категорию
                category_height = (SCREEN_HEIGHT - 120) // 4 - 10
                for i in range(4):
                    y_pos = 120 + i * (category_height + 10)
                    category_rect = pygame.Rect(10, y_pos, SCREEN_WIDTH - 20, category_height)
                    
                    if category_rect.collidepoint(event.pos):
                        game.active_category = i
                        game.scroll_start_x = mouse_pos[0]
                        break
            
            elif event.type == MOUSEMOTION:
                # Обработка прокрутки
                if game.active_category is not None and event.buttons[0]:
                    delta_x = mouse_pos[0] - game.scroll_start_x
                    if abs(delta_x) > SCROLL_SPEED:
                        direction = 1 if delta_x > 0 else -1
                        game.category_scroll[game.active_category] += direction * TASK_SIZE
                        game.scroll_start_x = mouse_pos[0]
                        
                        # Ограничиваем прокрутку
                        max_scroll = max(0, (game.tasks_per_category - 5) * (TASK_SIZE + 10) - (SCREEN_WIDTH - 40))
                        game.category_scroll[game.active_category] = max(0, min(max_scroll, game.category_scroll[game.active_category]))
            
            elif event.type == MOUSEBUTTONUP:
                game.active_category = None
